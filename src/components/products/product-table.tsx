...
import { getWeekdayDates } from "./make-data";
...
const ProductTable = () => {
  ...
  // Example usage:
  // Example usage:
  const startDate = "2024-12-01"; // Change to your desired start date
  const endDate = "2024-12-31"; // Change to your desired end date
  const dayToFilter = "Tuesday"; // Specify the day name to filter

  try {
    const mondayDates = getWeekdayDates(startDate, endDate, dayToFilter);
    console.log("gggggggggg", mondayDates);
  } catch (error) {
  ...
  useEffect(() => {
    const fetchData = () => {
      const payload = {
        ...
        limit: pageSize,
        categories: JSON.stringify([...filtersData?.selectedCategories]),
        vendors: JSON.stringify([...filtersData?.selectedVendors]),
      };
  ...
  return (
    <Fragment>
      ...
      {themeData?.featurDrawerView && <ViewProduct />}
    </Fragment>
  );
};
